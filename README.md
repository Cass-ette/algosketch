...
`algosketch` is a CLI that reads a single source file (Python, Java, C++, or Go) and produces:
...
`algosketch` renders language-neutral, CLRS-style pseudocode. Python, Java, C++, and Go inputs that represent the same algorithm produce identical pseudocode output.
...
`algosketch` 是一个命令行工具，读取一份 Python / Java / C++ / Go 源文件，生成：
...
`algosketch` 输出与具体编程语言无关、接近《算法导论》风格的伪代码。表达同一算法的 Python、Java、C++ 和 Go 输入会生成一致的伪代码结果。
