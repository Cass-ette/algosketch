...
| `x := expr`, `x = expr` | `Stmt::Assign` |
| `a, b = b, a` (multi-assign) | `Stmt::Assign` with tuple target/value (same as Python) |
| `var x T`, `var x T = v`, `const x = v` | `Stmt::VarDecl` |
| `if c { } else if { } else { }` | `Stmt::If` chains |
| `for i := 0; i < n; i++ { }` | `ForKind::CStyle` (i++ is the step expr, maps like C++) |
...
- CLI `CliLang::Go` → `--source-lang go`; `.go` auto-detected.
...
- Go fixtures for the 5 canonical algorithms (`binary_search`, `reverse_string`,
  ...
  skeleton) so cross-language equality holds.
...
- Parser-level diagnostics test: `go_reports_raw_statement_line` (a `defer` produces a
  Raw statement with the right line).
...
- [ ] `grep -rn "MIT" README.md Cargo.toml` → no license MIT references; LICENSE-MIT deleted.
